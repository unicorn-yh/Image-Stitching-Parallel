...
image_paths=['data/images/house/house1.jpg', 'data/images/house/house2.jpg']
...
for i in range(len(imgs)):
    ...
    cv2.imwrite('data/result/keypoints' + str(i) + ".jpg",cv2.drawKeypoints(imgs[i], kp, None))

end = time.time()

print("Find Keypoints Time: {:.5f} s".format(end - start))
...
for i in range(len(imgs)):
    ...
    for j in range(i+1, len(imgs)):
        ...
        cv2.imwrite("data/result/match" + str(i) + str(j) + ".jpg", res)

end = time.time()

print("Match Keypoints Time: {:.5f} s".format(end - start))
...
print("Homography Time: {:.5f} s".format(end - start))
...
plt.imshow(result)
plt.savefig('data/result/sequential-output.jpg')
